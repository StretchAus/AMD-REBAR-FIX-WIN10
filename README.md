...
3. The live logger screen will spit out verification receipts for all six optimization blocks before creating the silent background update fixer clone and scheduled task.
...
MCT2 HAS TO BE OPEN/MINIMISED TO WORK! DO NOT CLOSE IT!

I PERSONALLY RUN IT LIKE SO!

Once MCT2 is Downloaded:
...
3. Move shortcut from desktop into C:\Program Files\MCT2 (create the folder)

4. Create text file named asdf
Paste the following:

@echo off
:: Launch the shortcut silently
start "" "C:\Program Files\MCT2\MoreClockTool.lnk"
:: Wait 1 seconds for the app to load
timeout /t 1 /nobreak >nul
:: Force minimize using PowerShell (most reliable for UWP)
powershell -Command "Add-Type -AssemblyName UIAutomationClient; Add-Type -AssemblyName PresentationCore; $proc = Get-Process | Where-Object { $_.MainWindowTitle -like '*Clock*' -or $_.ProcessName -like '*Hellm*' }; if($proc) { $hwnd = $proc.MainWindowHandle; if($hwnd -ne [IntPtr]::Zero) { $ae = [System.Windows.Automation.AutomationElement]::FromHandle($hwnd); $wp = $ae.GetCurrentPattern([System.Windows.Automation.WindowPattern]::Pattern); $wp.SetWindowVisualState([System.Windows.Automation.WindowVisualState]::Minimized) } }"   

5. SAVE-AS - select all files from drop down - name it= MCT2 -minimised.bat

6. Copy MCT2 -minimised.bat from C:\Program Files\MCT2 into Startup
Press Win + R again: 
shell:startup

This Should now start minimised just pin to taskbar and move all the way left or whatever so you dont accidenatally open it up again
